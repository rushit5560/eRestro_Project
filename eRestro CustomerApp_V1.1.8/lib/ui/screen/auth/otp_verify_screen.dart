...
import 'package:erestro/ui/screen/auth/resendOtpTimerContainer.dart';
...
class OtpVerifyScreen extends StatefulWidget {
  ...
  const OtpVerifyScreen(
      {Key? key, this.countryCode, this.mobileNumber, this.from})
      : super(key: key);
...
class _OtpVerifyScreenState extends State<OtpVerifyScreen>
  ...
  login() async {
    await Future.delayed(
        ...
        () => context.read<SignInCubit>().signInUser(
            mobile:
                widget.mobileNumber /*, password: passwordController.text*/));
  }
  ...
  navigationPageRegistration() async {
    if (widget.from == "splash") {
      ...
      await Future.delayed(
          Duration.zero,
          () => Navigator.of(context).pushNamed(Routes.signUp, arguments: {
                'mobileNumber': widget.mobileNumber,
                'countryCode': widget.countryCode,
                'from': widget.from
              }));
        ...
        : BlocProvider<SignUpCubit>(
            ...
            child: Builder(
              builder: (context) => Scaffold(
                appBar: DesignConfig.appBar(context, width!, UiUtils.getTranslatedLabel(context, otpVerificationLabel), const PreferredSize(
