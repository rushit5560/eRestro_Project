...
import 'package:erestro/cubit/rating/setOrderRatingCubit.dart';
import 'package:erestro/cubit/rating/setProductRatingCubit.dart';
import 'package:erestro/data/model/orderModel.dart';
import 'package:erestro/data/model/rattingModel.dart';
...
import 'package:erestro/ui/widgets/ratingConatiner.dart';
...
import 'package:erestro/ui/screen/rating/thank_you_for_review_screen.dart';
...
class _ProductRatingScreenState extends State<ProductRatingScreen> {
  ...
  Widget orderData() {
    return BlocConsumer<OrderCubit, OrderState>(
        ...
        builder: (context, state) {
          ...
          return Container(
                    ...
                    child: Column(mainAxisAlignment: MainAxisAlignment.center, mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.center,
                      children: List.generate(orderList[0].orderItems!.length, (i) {
                        ...
                        return Container(
                            padding: EdgeInsetsDirectional.only(bottom: height! / 50.0, top: height!/ 50.0, start: width! / 20.0, end: width! / 20.0),decoration: DesignConfig.boxDecorationContainer(Theme.of(context).colorScheme.onBackground, 10.0),
                            //height: height!/4.7, 
                            width: width!,
                            margin: EdgeInsetsDirectional.only(
                                bottom: height! / 99.0, top: height! / 99.0),
                            child: Column(
                                mainAxisAlignment: MainAxisAlignment.center,
                                crossAxisAlignment: CrossAxisAlignment.center,
                                children: [
                                i==0?Text(UiUtils.getTranslatedLabel(context, howWasYourLabel),
                                    textAlign: TextAlign.center, style: TextStyle(color: Theme.of(context).colorScheme.onSecondary, fontSize: 14, fontWeight: FontWeight.w600)):const SizedBox(),
                                i==0?const SizedBox(height: 5.0):const SizedBox(),
                                i==0?Text(UiUtils.getTranslatedLabel(context, kindlyRateYourExperienceLabel),
                                    textAlign: TextAlign.center, style: const TextStyle(color: greayLightColor, fontSize: 12, fontWeight: FontWeight.w600)):const SizedBox(),
    ...
    return StatefulBuilder(builder: (BuildContext context, StateSetter setModalState) {
      return Container(
        padding: const EdgeInsetsDirectional.only(top: 5), margin: EdgeInsetsDirectional.only(top: height!/60.0),
        height: height!/10.0,
        child: Row(
          children: [
            InkWell(onTap: (){
                    _reviewImgFromGallery(setModalState);
                  },child: Align(alignment: Alignment.topLeft, child: SizedBox(width: reviewPhotos.isEmpty?width!/1.12:width!/4.0, height: height!/10.0,
                    child: DottedBorder(
  ...
  Widget build(BuildContext context) {
    ...
    return _connectionStatus == connectivityCheck
        ...
        : Scaffold(
            appBar: DesignConfig.appBar(context, width, UiUtils.getTranslatedLabel(context, giveFeedbackLabel), const PreferredSize(
            ...
            bottomNavigationBar: BlocConsumer<SetOrderRatingCubit, SetOrderRatingState>(
                ...
                builder: (context, state) {
                  return BlocConsumer<SetProductRatingCubit, SetProductRatingState>(
                    ...
                    builder: (context, state) {
                      return ButtonContainer(color: Theme.of(context).colorScheme.secondary, height: height, width: width, text: UiUtils.getTranslatedLabel(context, doneLabel), start: width! / 40.0, end: width! / 40.0, bottom: height! / 55.0, top: 0, status: status,borderColor: Theme.of(context).colorScheme.secondary, textColor: white, onPressed: (){
                                        setState((){
                                          status = true;
                                        });
                                        context
                                .read<SetOrderRatingCubit>()
                                .setOrderRating(context.read<AuthCubit>().getId(), widget.orderId, ratingList[selectedIndex!].rating, commentController.text, reviewPhotos);
                                        },);
                    });
              }
            ),
            body: Container(
                ...
                child: SingleChildScrollView(physics: const AlwaysScrollableScrollPhysics(),
                  child: Column(mainAxisSize: MainAxisSize.min,
                    children: [
                      Container(
                              padding: EdgeInsetsDirectional.only(bottom: height! / 99.0, top: height!/ 99.0, start: width! / 20.0, end: width! / 20.0),decoration: DesignConfig.boxDecorationContainer(Theme.of(context).colorScheme.onBackground, 10.0),
                              //height: height!/4.7, 
                              width: width!,
                              margin: EdgeInsetsDirectional.only(
                                  bottom: height! / 99.0, top: height! / 99.0),
                              child: Column(
                                  ...
                                  Text(UiUtils.getTranslatedLabel(context, howWasYourLabel),
                                      textAlign: TextAlign.center, style: TextStyle(color: Theme.of(context).colorScheme.onSecondary, fontSize: 14, fontWeight: FontWeight.w600)),
                                  const SizedBox(height: 5.0),
                                  Text(UiUtils.getTranslatedLabel(context, kindlyRateYourExperienceLabel),
                                      textAlign: TextAlign.center, style: const TextStyle(color: greayLightColor, fontSize: 12, fontWeight: FontWeight.w600)),
                                  const SizedBox(height: 30.9),
                                    ...
                                    child: Text(UiUtils.getTranslatedLabel(context, didYouLikeTheTasteLabel),
                                      ...
                                      Text(ratingList[selectedIndex!].title!, textAlign: TextAlign.center, style: TextStyle(color: Theme.of(context).colorScheme.primary, fontSize: 18, fontWeight: FontWeight.w600)),
                                      comment(),
                                      getImageField(),
                                  ])),
                      orderData(),
